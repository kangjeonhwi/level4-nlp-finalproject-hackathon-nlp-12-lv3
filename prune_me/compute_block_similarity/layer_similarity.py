...
def main(model_path: str, dataset: str, dataset_column: str, batch_size: int, max_length: int,
    ...
    config = LoraConfig(task_type=TaskType.CAUSAL_LM, inference_mode=True, r=4, lora_alpha=32, lora_dropout=0.1)
    model = AutoModelForCausalLM.from_pretrained(model_path,  
                                                device_map="auto", 
                                                torch_dtype=torch.float32,
                                                #  quantization_config=quantization_config, # macOS에서는 지원하지 않음
