...
def prepare_calibration_input(model, dataloader, device):
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            inps[cache['i']] = inp
            cache['i'] += 1
            cache['attention_mask'] = kwargs.get('attention_mask') or torch.ones(inp.shape[:2], device=inp.device, dtype=torch.long)
            cache['position_ids'] = kwargs.get('position_ids', torch.arange(inp.shape[1], device=inp.device).unsqueeze(0).expand(inp.shape[0], inp.shape[1]))
...
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    ...
    inps = load_samples_from_directory(inps_directory, nsamples=100, map_location="cuda:0")
    attention_mask = load_samples_from_directory(attn_directory, nsamples=100, map_location="cuda:0")
    
    print("inps result: ", inps)
    ...
    for i in tqdm(range(len(layers)), desc="레이어: "):
        ...
        for j in range(args.nsamples):
            with torch.no_grad():
                outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask.to(torch.bool), position_ids=position_ids, position_embeddings=position_embeddings)[0]
        inps, outs = outs, inps
